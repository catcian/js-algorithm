...
### 11-5 归并排序 /sort/mergeSort.js
firefox sort
思路
1. 分：把数组劈成两半，再递归对子数组进行“分”操作，直到分成一个个单独的数字
5，4，3，2，1
5，4
  5，
  4
...
const res = [1,2,3,4,5].sequentialSeach(3)
...
1. 效率比顺序搜索效率高
思路：
1. 从数组的中间元素开始，如果中间元素正好是目标值，则搜索结束，返回中间元素下标
...
步骤
1. 新建一个新链表，作为返回结果
1. 用指针遍历两个有序链表，并比较两个链表的当前节点，较小者先接入新链表，并将指针后移一步
1. 链表遍历结束，返回新链表
